...
describe('OrganizationsQuery', () => {
  ...
  it('should filter organizations', () => {
    ...
    expect(query.filterOrganizations(exampleOrganizations, 'po')).toEqual([{name: 'potato', approved: true}, {name: 'pork', approved: true}]);
    expect(query.filterOrganizations(exampleOrganizations, 'POTATO')).toEqual([{name: 'potato', approved: true}]);
    expect(query.filterOrganizations(exampleOrganizations, 'PO')).toEqual([{name: 'potato', approved: true}, {name: 'pork', approved: true}]);
    expect(query.filterOrganizations(exampleOrganizations, 'ck')).toEqual([{name: 'chicken', approved: true}, {name: 'duck', approved: true}]);
    expect(query.filterOrganizations(exampleOrganizations, 'CK')).toEqual([{name: 'chicken', approved: true}, {name: 'duck', approved: true}]);
  });
