...
describe('WorkflowLaunchService', () => {
  ...
  it('should get the wget test parameter file command', inject([WorkflowLaunchService], (service: WorkflowLaunchService) => {
    expect(service.getTestJsonString('github.com/garyluu/example_cwl_workflow', 'v1.0', 'cwl', 'test.json'))
      .toBe(`$ wget --header='Accept: text/plain' ` +
      `${Dockstore.API_URI}/api/ga4gh/v2/tools/#workflow/github.com/garyluu/example_cwl_workflow/versions/v1.0/PLAIN_CWL/descriptor/test.json ` +
      `-O Dockstore.json`);
    expect(service.getTestJsonString('github.com/garyluu/example_cwl_workflow', 'v1.0', 'wdl', 'test.json'))
      .toBe(`$ wget --header='Accept: text/plain' ` +
      `${Dockstore.API_URI}/api/ga4gh/v2/tools/#workflow/github.com/garyluu/example_cwl_workflow/versions/v1.0/PLAIN_WDL/descriptor/test.json ` +
      `-O Dockstore.json`);
    expect(service.getTestJsonString('github.com/garyluu/example_cwl_workflow', 'v1.0', 'nfl', 'test.json'))
      .toBe(`$ wget --header='Accept: text/plain' ` +
      `${Dockstore.API_URI}/api/ga4gh/v2/tools/#workflow/github.com/garyluu/example_cwl_workflow/versions/v1.0/PLAIN_NFL/descriptor/test.json ` +
      `-O Dockstore.json`);
