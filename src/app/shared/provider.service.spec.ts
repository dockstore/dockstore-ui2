...
describe('ProviderService', () => {
    ...
    it('should set up provider', inject([ProviderService], (service: ProviderService) => {
        ...
        expect(service.setUpProvider(tool, null).providerUrl).toBeFalsy();
        const tool2 = tool;
        tool2.gitUrl = 'git@github.com:denis-yuen/dockstore-tool-bamstats.git';
        expect(service.setUpProvider(tool2, null).providerUrl).toContain('https://github.com/');
        const tool3 = tool;
        tool3.gitUrl = 'https://garyluu@bitbucket.org/garyluu/dockstore-tool-md5sum.git';
        expect(service.setUpProvider(tool3, null).providerUrl).toContain('https://bitbucket.org/');
        const tool4 = tool;
        tool4.gitUrl = 'https://gitlab.com/garyluu/dockstore-tool-md5sum.git';
        expect(service.setUpProvider(tool3, null).providerUrl).toContain('https://gitlab.com/');
        const tool5 = tool;
        tool5.gitUrl = '';
        expect(service.setUpProvider(tool3, null).providerUrl).toBeFalsy();
        expect(service.setUpProvider(tool3, null).provider).toBeFalsy();
        const tool6 = tool;
