...
describe('ToolLaunchService', () => {
    ...
    it('should get the right check tool command', inject([ToolLaunchService], (service: ToolLaunchService) => {
        expect(service).toBeTruthy();
        expect(service.getCheckToolString('potato', null)).toBe('$ dockstore checker launch --entry potato checkparam.json');
        expect(service.getCheckToolString('potato', 'stew')).toBe('$ dockstore checker launch --entry potato:stew checkparam.json');
